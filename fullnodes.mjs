...
async function main() {
  const numNodes = 2; // Change the number of nodes as needed
  const nodes = await createNodes(numNodes);

  // Generate and store the premium wallet's keys
  const premiumWallet = new Wallet();
  await premiumWallet.initializePremium(); // Wait for initialization to complete
  console.log("Premium Wallet Address:", premiumWallet.publicKey);

  // Log premium wallet's private key and mnemonic
  ...
  // Store premium wallet information in a file
  const walletInfo = {
    publicKey: premiumWallet.publicKey,
    privateKey: premiumWallet.privateKey,
    mnemonic: premiumWallet.mnemonic,
  };

  const filePath = "./premiumWalletInfo.json";

  // Convert the walletInfo object to JSON format
  const walletInfoJson = JSON.stringify(walletInfo, null, 2);

  // Write the JSON data to a file
  fs.writeFileSync(filePath, walletInfoJson);
  nodes.forEach(async (node, index) => {
    ...
    node.services.pubsub.addEventListener("message", (message) => {
      ...
      if (checkTopic === "newTransactionTopic") {
        ...
        try {
          ...
          try {
            premiumWallet.signPremiumTransaction(
              premiumTransaction,
              premiumPrivateKey
            );
            // console.log(
            //  "signed transaction from premium wallet: ",
            //  premiumTransaction
            //);
    ...
    async function startConsensus(pool) {
      ...
      if (run.length > 0) {
        ...
        if (pendingTransactions.length == 0) {
          syncBlockchainOfOtherNodes(blockchain);
        }
      }
    }
    ...
    async function syncBlockchainOfOtherNodes(blockchain) {
      if (blockchain.chain.length > 1) {
        try {
          const syncMessage = JSON.stringify(blockchain.getAllBlocks());
          await node.services.pubsub.publish(
            "syncBlockchain",
            new TextEncoder().encode(syncMessage)
          );
        } catch (error) {
          console.log("coud not publish to syncBlockchain topic");
        }
      }
    }
